.PHONY: help install lock upgrade sync \
        ...
        data data-refresh data-info dev mcp-serve \
...
format: ## Format Python code
	uv run ruff format gencc_link tests server.py mcp_server.py scripts

format-check: ## Check formatting without writing
	uv run ruff format --check gencc_link tests server.py mcp_server.py scripts

lint: ## Lint Python code
	uv run ruff check gencc_link tests server.py mcp_server.py scripts

lint-ci: ## Lint with GitHub-Actions output
	uv run ruff check gencc_link tests server.py mcp_server.py scripts --output-format=github

lint-fix: ## Lint and apply safe fixes
	uv run ruff check gencc_link tests server.py mcp_server.py scripts --fix

lint-loc: ## Enforce per-file line budget (see AGENTS.md "File Size Discipline")
	uv run python scripts/check_file_size.py

typecheck: ## Type check package
	uv run mypy gencc_link server.py mcp_server.py

typecheck-fast: ## Type check with mypy daemon and fallback
	@tmp_log=$$(mktemp); \
	if uv run dmypy run -- gencc_link server.py mcp_server.py >$$tmp_log 2>&1; then \
		cat $$tmp_log; \
	else \
...
test-integration: ## Run integration tests (live GenCC download) serially
	uv run pytest tests -q -m "integration"

test-cov: ## Run tests with coverage
	uv run pytest tests -m "not integration" --cov=gencc_link --cov-report=term-missing --cov-report=html
...
data: ## Download GenCC export and build the SQLite database
	uv run gencc-link-data build

data-refresh: ## Refresh the database only if GenCC published a newer export
	uv run gencc-link-data refresh

data-info: ## Show build provenance for the local database
	uv run gencc-link-data info
