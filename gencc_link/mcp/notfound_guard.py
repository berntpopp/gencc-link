...
is materially lower-risk than the upstream-injection leak the prior sweep closed.
It is still worth closing: the reflected name/URI -- with any control/zero-width/
bidi/NUL code points -- lands in shared operator logs and in an agent's tool-result
context. Fixed constants remove the channel entirely.
...
* Layer 2 -- ``on_read_resource`` boundary: an unknown (URL-valid) resource makes
  ...
  URI-free ``ResourceError``. GenCC's resource handlers raise no author-authored
  ``ResourceError`` (they only return strings/JSON), so every exception here is a
  core not-found/read failure and is replaced unconditionally -- ``str(exc)``
...
Layer 4 (arg-validation) is the existing :class:`InputValidationMiddleware`
(``middleware.py``). Layer 6 (OTel span redaction) is a no-op here: gencc-link
declares no ``opentelemetry-sdk`` dependency, so FastMCP's tracer provider is the
API's non-recording proxy (spans never record, so no span exception attributes are
ever captured) -- there is nothing to redact (fleet policy: do NOT add the SDK).
"""
...
from gencc_link.mcp.envelope import _provenance_meta
...
def unknown_tool_envelope() -> dict[str, Any]:
    ...
    Built from constants only; ``_meta`` carries the shared provenance block and a
    discovery ``next_command`` but deliberately OMITS ``_meta.tool`` so the
    requested (caller-controlled) name is never reflected back. Shape matches this
    repo's structured error envelope (``success``/``error_code``/``message``/``_meta``).
    """
    return {
        "success": False,
        ...
        "_meta": {
            ...
            **_provenance_meta(is_error=True),
        },
    }
...
class NotFoundGuard(Middleware):
    ...
    async def on_call_tool(
        ...
        Only a fixed marker is logged -- the requested name is never logged.
        """
    ...
    async def on_read_resource(
        ...
        so the URI never reaches the caller/protocol. GenCC raises no
        author-authored ``ResourceError``, so every exception here is replaced and
...
def _is_structured_envelope(call_result: mcp.types.CallToolResult) -> bool:
    ...
    Distinguishes a structured gencc-link error (already input-free -- it carries an
...
def install_protocol_error_handler(mcp_server: Any) -> None:
    ...
    exist). A FastMCP core not-found (or read) error can then no longer reflect the
    caller-supplied name/URI.
    """
...
# The source loggers on which those records are CREATED. A logging filter runs only
# for records emitted on the logger it is attached to (ancestor filters are skipped
# during propagation, but HANDLER-level filters DO run during propagation), so
# attach directly to each originating logger -- including the ROOT logger, where
# ``mcp.shared.session`` emits its request-validation failures via a bare
# ``logging.warning``, and FastMCP's own non-propagating ``fastmcp`` logger, whose
# Rich handlers would otherwise bypass a root-only filter.
