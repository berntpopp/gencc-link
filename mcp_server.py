...
def main() -> None:
    """Run the GenCC-Link MCP server on the stdio transport."""
    # Configure environment BEFORE importing anything that may print.
    os.environ.setdefault("GENCC_LINK_TRANSPORT", "stdio")
    ...
    try:
        from gencc_link.logging_config import configure_logging
        from gencc_link.server_manager import UnifiedServerManager
    except Exception as exc:
        print(f"ERROR: gencc_link import failed: {exc}", file=sys.stderr)
        sys.exit(1)
